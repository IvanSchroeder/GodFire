...
public class CameraManager : Singleton<CameraManager> {
    ...
    void CalculatePanning() {
        _isDraggingCamera = Input.GetMouseButton(1);
        ...
        if (_isDraggingCamera) {
            ...
            _cameraTargetPosition += (_cameraMoveDirection * panSpeed / CalculatePercentOfZoom(_currentZoom) * Time.deltaTime).ToVector3();
        }

        _cameraTargetPosition.z = -cameraAltitude;
        // _cameraTargetPosition = _cameraTargetPosition.Clamp(new Vector3(-20, -20, -cameraAltitude), new Vector3(20, 20, -cameraAltitude));
        _cameraWorldPosition = Vector3.SmoothDamp(_cameraWorldPosition, _cameraTargetPosition, ref _cameraVelocity, (_isDraggingCamera ? dragSmoothness : panSmoothness) * Time.deltaTime);
    ...
    void PanCamera() {
        mainCamera.transform.position = _cameraWorldPosition;
